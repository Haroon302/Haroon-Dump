...
ua = random.choice([
    ...
    ua_galaxys10,
    ua_lenovo])
logo = "\x1b[0;91m\x01             CREATE FILE EASILY:)                 \x1b[0m\n-----------------------------------------------\n\x1b[0;91m \x1a OWNERS : Muhammad HAROON ~ MR HAROON THE GAMER \n\x1b[0;97m \x1a Github : Haroon302\n\x1b[0;91m \x1a Note   : Use  Kiwi Browser Token"
...
def login():
    ...
    try:
        ...
        if ___token___ in ('', ' '):
            exit('%s[%s!%s]%sYAAR PUT TOKEN HERE' % (P, M, P, M))
        xwx = requests.get('https://graph.facebook.com/me/?access_token=%s' % ___token___).json()
        print '%s[%s*%s]%s WelCome To HAROON %s %s' % (B, P, B, P, H, xwx['name'].lower())
...
def menu():
    ...
    print '\x1b[1;91mExample: /sdcard/Haroon.txt'
    print ''
